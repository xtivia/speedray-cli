...
describe('Acceptance: sr generate module', () => {
  setupProject();

  it('will fail if no name is specified', (done) => {
    return sr(['generate', 'module']).catch((error: string) => {
      expect(error).toBe('The `sr generate module` command requires a name to be specified.');
    })
    .then(done, done.fail);
  });

  it('sr generate module my-module', (done) => {
  ...
  it('sr generate module  generate routing file when passed flag --routing', (done) => {
  ...
  it('sr generate module my-module --spec', (done) => {
  ...
  it('sr generate module TwoWord', (done) => {
  ...
  it('sr generate module parent/child', (done) => {
    return sr(['generate', 'module', 'parent'])
      .then(() => sr(['generate', 'module', 'parent/child']))
      .then(() => {
  ...
  it('sr generate module child should work in sub-dir', (done) => {
    fs.mkdirSync(path.join(testPath, './sub-dir'));
    return new Promise(resolve => {
      process.chdir(path.join(testPath, './sub-dir'));
      return resolve();
    })
    .then(() => sr(['generate', 'module', 'child']))
    .then(() => {
  ...
  it('sr generate module child should work in sub-dir with routing file when passed --routing flag', (done) => {
    fs.mkdirSync(path.join(testPath, './sub-dir'));
    return new Promise(resolve => {
      process.chdir(path.join(testPath, './sub-dir'));
      return resolve();
    })
    .then(() => sr(['generate', 'module', 'child', '--routing']))
    .then(() => {
      expect(fs.pathExistsSync(path.join(testPath, 'sub-dir/child', 'child.module.ts'))).toBe(true);
  ...
  // tslint:disable-next-line:max-line-length
  it('sr generate module should generate parent/child module with routing file when passed --routing flag', (done) => {
    return sr(['generate', 'module', 'parent'])
      .then(() => sr(['generate', 'module', 'parent/child', '--routing']))
      .then(() => {
        expect(fs.pathExistsSync(path.join(testPath, 'parent/child', 'child.module.ts'))).toBe(true);
