...
xdescribe('Acceptance: sr generate route', function () {
  beforeEach(function () {
  ...
  it('sr generate route my-route', function () {
    return sr(['generate', 'route', 'my-route']).then(() => {
  ...
  it('sr generate route +my-route/my-other', () => {
    return sr(['generate', 'route', '+my-route'])
      .then(() => sr(['generate', 'route', '+my-route/my-other', '--default']))
      .then(() => sr(['generate', 'route', '+my-route/+my-other/my-third', '--default']))
      .then(() => {
  ...
  it('sr generate route details --path /details/:id', () => {
    return sr(['generate', 'route', 'details', '--path', '/details/:id'])
      .then(() => {
        const appContent = fs.readFileSync(path.join(testPath, 'foo.component.ts'), 'utf-8');
  ...
  it('sr generate route my-route --dry-run does not modify files', () => {
    ...
    return sr(['generate', 'route', 'my-route', '--dry-run']).then(() => {
